...
class Order(models.Model):
    ...
    address = models.ForeignKey(Address, on_delete=models.SET_NULL, null=True, blank=True)  # shipping
    total_price = models.DecimalField(max_digits=8, decimal_places=2)         # total amount
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='Pending')  # status
    ordered_at = models.DateTimeField(auto_now_add=True)                       # created
    updated_at = models.DateTimeField(auto_now=True)                           # last updated
...
class OrderItem(models.Model):
    ...
    price = models.DecimalField(max_digits=8, decimal_places=2)                        # price at order
